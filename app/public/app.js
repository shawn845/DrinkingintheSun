...
function getDetailGalleryItems(pub) {
  ...
  if (pub.imageUrl) {
    items.push({ src: pub.imageUrl, alt: pub.name, label: '' });
  }
  if (pub.spotAPhotoUrl) {
    items.push({ src: pub.spotAPhotoUrl, alt: `${pub.name} ${pub.spotA || 'Spot A'}`, label: pub.spotA || 'Spot A' });
  }
  if (pub.spotBPhotoUrl) {
    items.push({ src: pub.spotBPhotoUrl, alt: `${pub.name} ${pub.spotB || 'Spot B'}`, label: pub.spotB || 'Spot B' });
  }
...
function renderDetailGallery(pub) {
  ...
  if (items.length === 1) {
    const item = items[0];
    return `<div class="detailGalleryWrap"><img class="heroImg" loading="lazy" referrerpolicy="no-referrer" src="${escapeAttr(item.src)}" alt="${escapeAttr(item.alt)}" onerror="this.style.display='none';" />${item.label ? `<div class="detailGalleryLabel">${escapeHtml(item.label)}</div>` : ''}</div>`;
  }

  return `
    <div class="detailGallery" aria-label="Pub photos" tabindex="0">
      ${items.map(item => `
        <figure class="detailSlide">
          <img class="heroImg detailSlideImg" loading="lazy" referrerpolicy="no-referrer" src="${escapeAttr(item.src)}" alt="${escapeAttr(item.alt)}" onerror="this.closest('.detailSlide').style.display='none';" />
          ${item.label ? `<figcaption class="detailGalleryLabel">${escapeHtml(item.label)}</figcaption>` : ''}
        </figure>
      `).join('')}
    </div>
  `;
